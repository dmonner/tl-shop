...
public class InternalDomain
{
	 ...
	 * external code calls that must be imported before being used in the domain
	 * description.
	 */
	private Vector calcs;

	/**
	 * A <code>Vector</code> of <code>String</code> names of compund tasks
	 ...
	 * number of constant symbols in the planning problem more than this indicates
	 * presence of constant symbols that appear exclusively in the problem
	 * description.
	 */
	 ...
	 * description to the list of constant symbols, in case it has not been added
	 * before.
	 * 
	 * @param s
	 *          the <code>String</code> to be added.
	...
	/**
	 * This function writes the Java code necessary to produce this domain at run
	 * time in the appropriate file.
	 * 
	 * @param varsMaxSize
	 *          the maximum number of variables seen in any variable scope in this
	 *          domain.
	 * @throws IOException
	 */
	public void close(int varsMaxSize) throws IOException
	{
		...
		for(int i = 0; i < calcs.size(); i++)
		{
			...
			s += "\tpublic static " + imp + " calculate" + imp + " = new " + imp
			  + "();" + endl + endl;
		}
	...
	/**
	 * This function performs some necessary initialization when a problem file is
	 * being compiled, mainly reading and parsing the text file associated with
	 * the domain the planning problem is defined in.
	 * 
	 * @throws IOException
	 */
	 ...
	 * @throws IOException
	 */
	public void commandToCode(LinkedList states, LinkedList taskLists)
	  throws IOException
	{
		// -- To hold the String to be written.
		String s;
		...
		// -- symbols.
		s += "\t\tTermConstant.initialize(" + constants.size() + ");" + endl + endl;
		...
		while(e.hasNext())
		{
			...
			s += "\t\twhile (thread.isAlive())" + endl + "\t\t\tThread.sleep(500);"
			  + endl;
	 ...
	 * 
	 * @param list
	 *          the <code>Vector</code> the elements of which are to be stored
